        ...
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
            <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#e6f4ff', padding: '16px 24px', borderRadius: 12 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                    <Text strong style={{ display: 'block' }}>各实验组流量总计: {totalRatio}% / 100%</Text>
                        ...
                        title={
                            <Space>
                                <Input
                                    ...
                                    disabled={isLocked}
                                />
                                {group.isControl ? <Tag color="default">对照组 (Control)</Tag> : <Tag color="blue">实验组 (Variation)</Tag>}
                            </Space>
                        }
                    ...
                    >
                        <Row gutter={48}>
                            <Col span={9}>
                                <Form.Item label={<Text strong>流量比例 (%)</Text>} required>
                                    <InputNumber
                                        ...
                                        addonAfter="%"
                                        disabled={isReadOnly} // Only ratio stays editable if isOngoing (not isReadOnly)
                                    />
                                </Form.Item>
                            ...
                            <Col span={15} style={{ borderLeft: '1px solid #f0f0f0' }}>
                                <Form.Item
                                    label={<Text strong><RocketOutlined /> 绑定配置实验值 (Variation)</Text>}
                                    required
                                    extra={selectedFeature ? `请选择 ${selectedFeature.name} 下的变体` : "请先在第一步选择 Feature"}
                                >
                                    <Select
                                        placeholder="请选择变体"
                                ...
                                {group.variationId ? (
                                    <div style={{ marginTop: 12, padding: '16px', background: '#f6ffed', borderRadius: 12, border: '1px solid #b7eb8f', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <Space direction="vertical" size={2}>
                                            <Text type="secondary" style={{ fontSize: 12 }}>已选变体摘要：</Text>
